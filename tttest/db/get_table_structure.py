...
def get_table_structure(table_name,project):
    ...
    if table_name == 'all':
        query = "SHOW TABLES"
        cursor =  init_db(project).execute_sql(query)
        tables = cursor.fetchall()
        return [[table[0],get_table_columns(table[0],project)] for table in tables]
